...
module.exports = {
    addCategory: async(req, res) => {
        try {
            ...
            if (!categoryName) {
                return handleResponse({ res, msg: "Please Enter categoryName" });
            }
            ...
            if (category) {
                ...
                return handleResponse({ res, msg: "Category created successfuly", data: newCategory });
            }
        } catch (error) {
    ...
    getAllCategory: async(req, res) => {
        try {
            ...
            if (!categoryData) {
                return handleResponse({ res, msg: "No category Available" });
            } else {
                return handleResponse({ res, msg: "Category List ", data: categoryData });
            }
        } catch (error) {
            return handleError({ res, error, data: error });
        }
    ...
    getSingleCategory: async(req, res) => {
        try {
            ...
            if (!categoryId) {
                return handleResponse({ res, msg: "Please provide categoryId in params" });
            } else {
                const singleCegtegoryData = await getSingleCategory(categoryId);
                if (!singleCegtegoryData) {
                    return handleResponse({ res, msg: "No category Available for given Id" });
                } else {
                    return handleResponse({ res, msg: "Single Category Data", data: singleCategoryData });
    ...
    updateCategory: async(req, res) => {
        try {
            ...
            const { categoryName } = req.body;
            const value = await categoryJoi.validateAsync({ categoryName });
            if (!categoryId) {
                return handleResponse({ res, msg: "Please provide categoryId in params" });
            } else {
                const category = await findCategoryByName(value);
                if (category) {
                    return handleResponse({ res, msg: "Category already Registered" })
                } else {
    ...
    deleteCategory: async(req, res) => {
        try {
            ...
            if (!category) {
                return handleResponse({ res, msg: "No data availabe for the ID provided" });
            } else {
